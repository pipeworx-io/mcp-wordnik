...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Wordnik');
}
...
const tools: McpToolExport['tools'] = [
  {
    name: 'definitions',
    description: 'Fetch dictionary definitions for a word from Wordnik; filter by part of speech, source dictionary, and limit. Returns definitions with source, text, and part of speech.',
  ...
  { name: 'top_example', description: 'Return the single highest-quality usage example sentence for a word from Wordnik\'s corpus.', inputSchema: { type: 'object', properties: { word: { type: 'string' }, useCanonical: { type: 'boolean' } }, required: ['word'] } },
  { name: 'related', description: 'Return words related to a given word by relationship type (synonym, antonym, hypernym, hyponym, etc.) from Wordnik\'s relatedWords endpoint.', inputSchema: { type: 'object', properties: { word: { type: 'string' }, relationshipTypes: { type: 'string' }, limitPerRelationshipType: { type: 'number' }, useCanonical: { type: 'boolean' } }, required: ['word'] } },
  { name: 'phrases', description: 'Return common bigram phrases containing a word from Wordnik\'s corpus; wlmi parameter sets minimum mutual-information threshold.', inputSchema: { type: 'object', properties: { word: { type: 'string' }, limit: { type: 'number' }, wlmi: { type: 'number' }, useCanonical: { type: 'boolean' } }, required: ['word'] } },
  { name: 'pronunciations', description: 'Return pronunciation strings (IPA, ahd, etc.) for a word from Wordnik; filter by source dictionary and format type.', inputSchema: { type: 'object', properties: { word: { type: 'string' }, sourceDictionary: { type: 'string' }, typeFormat: { type: 'string' }, useCanonical: { type: 'boolean' }, limit: { type: 'number' } }, required: ['word'] } },
  { name: 'hyphenation', description: 'Return hyphenation breakpoints and syllable stress for a word from Wordnik\'s hyphenation endpoint.', inputSchema: { type: 'object', properties: { word: { type: 'string' }, sourceDictionary: { type: 'string' }, useCanonical: { type: 'boolean' }, limit: { type: 'number' } }, required: ['word'] } },
  { name: 'frequency', description: 'Return year-by-year corpus frequency counts for a word from Wordnik; optionally constrain to a startYear–endYear range.', inputSchema: { type: 'object', properties: { word: { type: 'string' }, useCanonical: { type: 'boolean' }, startYear: { type: 'number' }, endYear: { type: 'number' } }, required: ['word'] } },
  { name: 'word_of_the_day', description: 'Fetch the Wordnik Word of the Day for a given date (YYYY-MM-DD) or today if omitted; returns word, definitions, and example sentences.', inputSchema: { type: 'object', properties: { date: { type: 'string' } } } },
  {
    name: 'random_word',
    description: 'Return a single random word from Wordnik; filter by part of speech, min/max length, and minimum corpus or dictionary occurrence count.',
  ...
  {
    name: 'random_words',
    description: 'Return up to `limit` random words from Wordnik; filter by part of speech, min/max length, and minimum corpus or dictionary occurrence count.',
  ...
  {
    name: 'search',
    description: 'Search the Wordnik dictionary by query string with wildcard support; filter by part of speech, length, and corpus count. Returns matching word strings.',
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (!apiKey) throw new Error('Wordnik requires an API key: pass your key as the _apiKey argument (free at https://developer.wordnik.com/).');
  const get = async (path: string, extras: Record<string, unknown> = {}) => {
    ...
    if (!res.ok) throw await httpError(res, 'Wordnik');
    return res.json();
  };
